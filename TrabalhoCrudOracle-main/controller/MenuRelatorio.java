...
public class MenuRelatorio {
    public static void menuRelatorio(Scanner teclado, Connection connection) {
        ...
        String query = "SELECT f.funcionario_id, f.nome, f.telefone, p.data_registro, p.hora_entrada, p.hora_saida, p.horas_trabalhadas " +
                       "FROM Funcionario f LEFT JOIN Ponto p ON f.funcionario_id = p.funcionario_id";

        try (PreparedStatement pstmt = connection.prepareStatement(query);
             ResultSet rs = pstmt.executeQuery()) {

            System.out.printf("%-15s %-30s %-20s %-15s %-15s %-15s %-20s%n", "ID", "Nome", "Telefone", "Data Registro", "Hora Entrada", "Hora Saída", "Horas Trabalhadas");
            System.out.println("----------------------------------------------------------------------------------------------------------------");
